...
| 2 | **HRX with Vulkan.** llama.cpp with `GGML_HRX=ON` and `GGML_VULKAN=ON` in one build, on AMD's tested pair | `AMD-Ecosystem/llama.cpp` `hrx-graph-develop-v2` + `ROCm/hrx-system`, pinned from `ROCm/ggml-staging-automation`; our fixes in the private `1bit-MONSTER/gpu-kernels` | **landed** ([docs/hrx.md](hrx.md)): `1bit serve` runs the same checkpoint on `Vulkan0` and `HRX0`; kept current by `bump-hrx.yml`. Our GPU kernel work (MoE above 128 experts, the prefill split, ROCmI4 on Vulkan) is closed source and built in with `-DONEBIT_GPU_PRIVATE` ([docs/hrx.md](hrx.md#private-gpu-build)) |
...
- **Vulkan is not inside HRX.** One build compiles both ggml backends.
  ...
  - Q4NX loaded only on `HRX20`, the ggml-hrx2 backend AMD dropped in June. Since the
    ...
    kept in a private archive for a port.
- **NPU concurrency:**
