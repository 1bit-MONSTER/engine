...
| 3x | **35B MoE on the NPU (experimental).** The whole 40-layer Qwen3.6-35B-A3B token as one runlist submit, experts routed on the device | `third_party/OpenFlowLM-Next` `open_kernels/designs/layer_x/lax.py`, `scripts/build-lax.sh` | **landed as experimental** ([docs/npu-lax.md](npu-lax.md)): kernels built from the pinned source, `tests/npu_lax_parity.sh` passes (3 positions vs fp64, argmax 846 / 198 / 3710), 16.1 tok/s chat. C++ driver `1bit npu-lax` (no Python): packer byte-identical to the Python one on all 83 buffers, same parity, ready in 3.6-3.8 s. **Full ELFs** (the default; no xclbin opened): the seven ELFs byte-identical to the reference harness's, logits bit-identical to the classic path, 16.3-16.5 tok/s decode vs 16.0-16.1 classic; `1bit serve --device npu` routes the model there and answers "The capital of France is Paris." (`tests/npu_lax_serve.sh`). **Open:** batched prefill; memory (22 GB pinned) |
